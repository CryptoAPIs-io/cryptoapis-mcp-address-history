...
const CRYPTOAPIS_SERVER_INFO = {
    name: "cryptoapis-address-history",
    version: "0.1.0",
    title: "CryptoAPIs Address History",
...
// Build an MCP server instance with tools, resources, and prompts registered
...
export async function startAddressHistoryServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        logger.logInfo("cryptoapis-address-history MCP running (stdio)");
        return;
    }
    ...
    app.listen(port, host, () => {
        logger.logInfo(`cryptoapis-address-history MCP running (http) at http://${host}:${port}${path}`);
